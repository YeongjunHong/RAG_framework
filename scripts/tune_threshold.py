...
warnings.filterwarnings("ignore")
logging.basicConfig(level=logging.ERROR, force=True)
for logger_name in ["httpcore", "httpx", "datasets", "urllib3", "fsspec", "filelock", "sentence_transformers", "huggingface_hub", "src.rag"]:
    logging.getLogger(logger_name).setLevel(logging.ERROR)
...
async def evaluate_thresholds():
    ...
    # [MODIFIED] 애플리케이션 컨텍스트에 맞는 Pool 생명주기 관리
    async with asyncpg.create_pool(dsn=async_db_url, min_size=5, max_size=20) as pool:
        retriever = PostgresHybridRetriever(pool=pool)
        reranker = LocalCrossEncoderReranker()
        filter_config = FilteringConfig()
        ...
        for intent, items in datasets_by_intent.items():
            ...
            # 검색 및 리랭킹 속도를 높이기 위한 병렬 Task 구성
            async def process_item(item):
                query = item["query"]
                target_ids = set(item["target_ids"])
                retrieved = await retriever.forward(queries=[query], top_k=40)
                reranked = await reranker.forward(query=query, candidates=retrieved, top_k=20)
                return target_ids, reranked
            
            # 병렬 처리로 DB 및 로컬 모델 워크로드 최적화
            batch_results = await asyncio.gather(*(process_item(item) for item in items))
            
            for target_ids, reranked_chunks in batch_results:
                for fl in floor_candidates:
                    kept_chunks = simulate_hybrid_filter(reranked_chunks, fl, rule)
                    kept_source_ids = set(c.chunk.source_id for c in kept_chunks)
                    results[fl]["tp"] += len(kept_source_ids.intersection(target_ids))
            ...
            for fl in floor_candidates:
                ...
                if f1 > best_f1:
                    best_f1 = f1
                    best_fl = fl
                elif f1 == best_f1 and f1 > 0 and fl < best_fl:
                    best_fl = fl
