...
warnings.filterwarnings("ignore")
logging.basicConfig(level=logging.ERROR, force=True)
for logger_name in ["httpcore", "httpx", "sentence_transformers", "urllib3", "openai", "ragas", "ragas._analytics"]:
    logging.getLogger(logger_name).setLevel(logging.ERROR)
...
async def _process_single_eval(app, item: dict, semaphore: asyncio.Semaphore) -> dict:
    """단일 평가 데이터 실행을 위한 코루틴 (Semaphore로 동시성 제어)"""
...
async def run_evaluation():
    print("RAG 파이프라인 가동 및 평가 데이터 병렬 수집 중...\n")
    app = build_graph()

    # 루프 진입 전 캐시 1회 명시적 초기화
    SemanticCacheManager(host="localhost", port=6379).clear_cache()
    
    # 동시성 제어: 한 번에 최대 10개의 요청만 파이프라인에 인입
    semaphore = asyncio.Semaphore(10)
    
    # 비동기 병렬 처리로 수집 시간 극단적 단축
    tasks = [_process_single_eval(app, item, semaphore) for item in EVAL_DATASET]
    ...
    evaluator_embeddings = HuggingFaceEmbeddings(model_name="jhgan/ko-sroberta-multitask")
