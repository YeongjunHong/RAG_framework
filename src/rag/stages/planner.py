...
#         strict_validation = plan.get("strict_validation", False) # 추가 
...
#         # --- 신규: 스트리밍 및 Post-Check 모드 제어 ---
#         # 1. 클라이언트가 스트리밍을 껐거나, 2. Planner가 엄격한 검증(authoring)이 필요하다고 판단한 경우
#         if not request.stream_response or strict_validation or intent == "authoring":
#             ctx.is_streaming = False
#             ctx.strict_validation = True
#             ctx.stream_queue = None # Generator에서 큐로 쏘는 것을 원천 차단
#             logger.info("[PlannerStage] Authoring 모드 감지: 스트리밍을 비활성화하고 Strict Post-Check을 준비합니다.")
#         else:
